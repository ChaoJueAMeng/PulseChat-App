...
onLoad((q) => {
  ...
  if (q.title) {
    groupTitle.value = decodeURIComponent(q.title)
    editTitle.value = groupTitle.value
  }
})
  ...
  uni.chooseImage({
    ...
    success: async (res) => {
      ...
      try {
        ...
        await api.updateGroup(conversationId.value, { avatar: url })
        avatar.value = url
...
async function saveGroupInfo() {
  ...
  saving.value = true
  try {
    await api.updateGroup(conversationId.value, {
      title,
...
function goMembers() {
  uni.navigateTo({
    url: '/pages/group-members/group-members?id=' + conversationId.value
...
async function onNotifyChange(e) {
  // switch checked = 开启通知 → muted = !checked
  const enable = !!e.detail.value
  const nextMuted = !enable
  try {
    await api.updateConvSettings(conversationId.value, { mute: nextMuted ? 1 : 0 })
